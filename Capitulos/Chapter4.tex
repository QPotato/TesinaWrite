...
\begin{proof}
    ...
    Sea $(i^\tau, j^\tau)_{\tau \in \mathbb{N}}$ una secuencia de juego ficticio secuencial sobre A, con secuencias de creencias $(x^\tau, y^\tau)_{\tau \in \mathbb{N}}$. En la iteración $\tau$, sabemos que $i^\tau \in BR_1(y^{\tau - 1})$ en $A$. Como el pago de $k+1$ para el jugador $1$ es siempre el mínimo de la columna, para cualquier acción $j$ del jugador $2$ tendremos que $a_{i^\tau, j} > a_{k+1,j}$ y por tanto podemos afirmar que $i^\tau \in BR_1(y^{\tau - 1})$ en $A'$.

    Para $j^\tau$ podemos razonar de forma análoga, solo que considerando la pertenencia a $BR_2(x^{\tau - 1})$ o $BR_2(x^\tau)$ según si hablamos de juego ficticio secuencial o alternante. \duda{asi o detallo mas?}

    Podemos afirmar entonces que $(i^\tau, j^\tau)_{\tau \in \mathbb{N}}$ es también una secuencia de juego ficticio (secuencial o alternante) válida en $A'$.
\end{proof}
...
\begin{lemma}
    % k_1, k_2
    Sea $(A, B)$ un juego representable $O(k)$, de tamaño $n \times m$ con $n, m < k$, entonces existe un juego $(A', B')$ de tamaño $k \times k$ tal que toda secuencia $(x^\tau, y^\tau)_{\tau \in \mathbb{N}}$ es un proceso de juego ficticio simultaneo en $(A, B)$ sí y solo sí lo es también en $(A', B')$.
\end{lemma}

\begin{proof}
    asd \comentario{Mi idea es demostrar estos expandiendo con las acciones con minima ganancia y ver el proceso sigue igual porque no hay incentivo para jugar esas salvo empates.}
\end{proof}

\begin{lemma}
    Sea $(A, B)$ un juego representable $O(k)$, de tamaño $n \times m$ con $n, m < k$, entonces existe un juego $(A', B')$ de tamaño $k \times k$ tal que toda secuencia $(x^\tau, y^\tau)_{\tau \in \mathbb{N}}$ es un proceso de juego ficticio alternante en $(A, B)$ sí y solo sí lo es también en $(A', B')$.
\end{lemma}
\cambios{Este parrafo y estos lemas. No estoy seguro aun de si es la mejor formulación o de si así aportan.}
...
\begin{theorem}
    Existe un juego simétrico $A$ representable en $O(k)$ bits y una secuencia de juego ficticio alternante $(x^\tau, y^\tau)_{\tau \in \mathbb{}}$
