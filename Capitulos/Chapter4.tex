...
\section{Convergencia del juego ficticio} \label{sec:convergencia:fp}
...
% Intro al lema
Presentamos a continuación un lema que muestra cómo podemos aplicar una transformación que expanda un juego de ganancias idénticas cumpliendo estas condiciones.

\begin{lemma}\label{lema:preservacion}
    Sea $A$ un juego de ganancias idénticas de tamaño $n\times m$ con un único equilibrio de Nash puro, y sean $n'\geq n$, $m'\geq m$. Entonces existe un juego de ganancias idénticas $A'$ de tamaño $n'\times m'$ y rango de imagen $O(n'+m')$ con un único equilibrio de Nash puro tal que:
    \begin{itemize}
...
\begin{proof}
    ...
    Para obtener un juego de ganancias idénticas de tamaño $n \times (m+1)$ que cumpla las propiedades deseadas, podemos definir $A'$ como:
    ...
    Finalmente, se itera la construcción anterior $n'-n+m'-m$ veces para obtener un juego de ganancias idénticas de $n'\times m'$ con las propiedades deseadas.
...
\begin{theorem} \label{teorema:afp:mejor}
    Para todo $k \geq 2$, $n\geq 2$, $m\geq 3$, existe un juego no degenerado de ganancias idénticas (y por tanto, de intereses idénticos) $A$ de $n\times m$ representable en $O(k+nm. log(max\{n,m\}))$ bits, con rango de imagen $O(n+m)$, y una secuencia de juego ficticio simultáneo $(i^\tau, j^\tau)_{\tau \in \mathbb{N}}$ sobre $A$ tal que, $(i^{2^k}, j^{2^k})$ no es un equilibrio de Nash puro y para todo proceso de juego ficticio alternante $(\widehat{i}^\tau, \widehat{j}^\tau)_{\tau \in \mathbb{N}}$ en $A$, $(\widehat{i}^4, \widehat{j}^4)$ es un equilibrio de Nash puro.
\end{theorem}
\begin{proof}
    Sea $A$ el juego de ganancias idénticas siguiente, con $\epsilon < 1$:
