...
\begin{proof}
    ...
    Pasemos ahora al caso simultaneo, para el cual nos basaremos en la prueba de Brandt, Fischer Y Harrenstein. Si el proceso comienza con el perfil $(i^1, j^1)$, entonces las utilidades esperadas serán $Ax^1 = (1, 0)$ e $y^1 B = (1, 2, 0)$ respectivamente. Luego, en la segunda iteración el jugador fila elegirá $i^1$ y el jugador columna $j^2$. Las utilidades esperadas se actualizaran entonces como $Ax^2 = (\frac{3}{2}, \frac{2 + 2^{-k}}{2})$ e $y^2 B = (1, 2, 0)$.
    A continuación, por al menos $2^k$ rondas, los jugadores elegiran las mismas jugadas que en la iteración 2, dado que para todo $\tau$ tal que $2 \le i \le 2^k$, tendremos $Ax^\tau = (2 - \frac{1}{\tau}, 2 - \frac{2 + 2^{-k}}{\tau})$ e $y^\tau B = (1, 2, 0)$.
    Luego, la secuencia de perfiles

    \begin{center}
    \begin{math}
        (i^1, j^1), \underbrace{(i^1, j^2), ... (i^1, j^2)}_{\text{$2^k$ veces}}
    \end{math}
    \end{center}
