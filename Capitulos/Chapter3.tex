...
\begin{definition}
    Un juego $(A, B)$ de tamaño $n \times n$ es \concept{simétrico} si $N = M$ y para todo $i, j \in N$, $a_{ij} = b_{ji}$.
\end{definition}

\begin{definition}
    Un juego $(A, B)$ de tamaño $n \times m$ es \concept{de intereses idénticos} si es BR-equivalente en estrategias mixtas a un juego con matrices de pago idénticas. Es decir, si existe un juego $(C, C)$ de tamaño $n \times m$ tal que:
    \begin{itemize}
    ...
    \end{itemize}
    Para estos casos también, nos referiremos al juego usando solo la matriz $A$.
