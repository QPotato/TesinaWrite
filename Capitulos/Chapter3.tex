...
\begin{definition}
    Un juego $(A, B)$ de tamaño $n \times n$ es un \concept{juego simétrico} si es de suma cero y, para todo $i, j \in N$, $a_{ij} = -a_{ji}$ y $b_{ij} = -b_{ji}$. En este caso también nos referiremos al juego usando solo la matriz $A$.
\end{definition}

\begin{definition}
    Un juego $(A, B)$ de tamaño $n \times m$ es un \concept{juego de intereses idénticos} si para todo $i \in N$ y $j \in M$, $a_{ij} = b_{ij}$. En este caso también nos referiremos al juego usando solo la matriz $A$.
\end{definition}
